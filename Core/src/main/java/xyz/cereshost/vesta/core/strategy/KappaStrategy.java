...
public class KappaStrategy implements TradingStrategy {
    ...
    private static final int DMI_LENGTH = 14;
    private static final int ADX_SMOOTHING = 14;
    private static final double ADX_THRESHOLD = 12.0;
    private static final double RSI_LONG_THRESHOLD = 55.0;
    private static final double RSI_SHORT_THRESHOLD = 45.0;

    private static final double TP_POINTS = 80.0;
    private static final double SL_POINTS = 60.0;
    private static final double TICKS_PER_POINT = 10.0;
    private static final double PRICE_TICK_SIZE = 0.01;

    private static final boolean USE_TRAILING_STOP = true;
    private static final double TRAIL_POINTS_MULTIPLIER = 0.5;
    ...
    @Override
    public void executeStrategy(PredictionEngine.PredictionResult pred, List<Candle> visibleCandles, Trading operations) {
        ...
        double rsi = computeLatestRsi(visibleCandles, RSI_LENGTH);
        ...
        if (!operations.hasOpenOperation()) {
            if (buyCondition) {
                openOperation(operations, Trading.DireccionOperation.LONG, current.close());
            } else if (sellCondition) {
                openOperation(operations, Trading.DireccionOperation.SHORT, current.close());
            }
        }
    ...
    private void openOperation(Trading operations, Trading.DireccionOperation direction, double entryPrice) {
        ...
        double tpTicks = TP_POINTS * TICKS_PER_POINT;
        double slTicks = SL_POINTS * TICKS_PER_POINT;
        ...
        Trading.OpenOperation open = operations.open(tpPercent, slPercent, direction, amountUsdt, LEVERAGE);
        if (open != null) {
            trailingStates.put(open.getUuid(), new TrailingState(open.getEntryPrice()));
        }
    }

    private void applyTrailingStop(Trading operations, Candle current) {
    ...
    private void syncTrailingState(Trading operations) {
